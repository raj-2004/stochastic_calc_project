...
def perform_test():
    ...
    def exact_solution_mean(X0, T, W_T):
        mean = X0 + T + (T**2)/4 - 2 * np.sqrt(X0) * T
        # return (np.sqrt(X0) + W_T - T / 2) ** 2
        return mean

    # Euler-Maruyama method
    def euler_maruyama_mean(X0, T, N, M):
        dt = T / N
    ...
    for N in steps_list:
        ...
        X_true = exact_solution_mean(X0, T, W_T)

        # Simulate approximation
        X_approx = euler_maruyama_mean(X0, T, N, M)

        # Compute mean absolute error
        error = np.abs(X_true - X_approx)
        errors.append(error)
